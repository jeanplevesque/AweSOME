    ...
    public class KryptonRenderHelper
    {
        ...
        public void BufferAddShadowHull(ShadowHull hull)
        {
            ...
            for (int i = 0; i < hull.NumPoints; i++)
            {
                ...
                Matrix.CreateRotationZ(hull.Angle, out matrixRotation);
                Matrix.CreateTranslation(hull.Position.X, hull.Position.Y, 0f, out matrixTranslation);
                Matrix.CreateScale(hull.Scale.X, hull.Scale.Y, 0, out matrixScale);
                Matrix.CreateScale(1f / hull.Scale.X, 1f / hull.Scale.Y, 0, out matrixScaleInv);
                ...
                this.mShadowHullVertices.Add(hullVertex);
            }

            // Add the indicies to the buffer
            foreach (int index in hull.Indicies)
            {
                mShadowHullIndicies.Add(vertexCount + index);
            }
        }
        ...
        public void DrawSquareQuad(Vector2 position, float rotation, float size, Color color)
        {
            ...
            size = (float)Math.Sqrt(size*size*2);
